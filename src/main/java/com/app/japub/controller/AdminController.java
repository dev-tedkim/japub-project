...
public class AdminController {
	...
	@ResponseBody
	public ResponseEntity<Void> commentDelete(@PathVariable Long commentNum) {
		Long userNum = (Long) session.getAttribute(SessionUtil.KEY);
		boolean isAdmin = SessionUtil.isAdmin(session);
		if (userNum == null || !isAdmin || commentNum == null) {
			return ResponseEntity.badRequest().build();
		}
		if (!adminService.deleteByCommentNum(commentNum)) {
			return ResponseEntity.badRequest().build();
		}
